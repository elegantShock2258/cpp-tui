...
- **Key Bindings**: `register_key(int key, callback, bool ctrl=false, bool alt=false, bool shift=false)` registers a global keyboard shortcut with optional modifiers. The callback fires before widget-level dispatch, giving registered keys priority over focused widgets. Supports any combination of Ctrl, Alt, and Shift. See `examples/key_events.cpp` for a working demo.
- **Timers**:
...
Events are handled by the `App` and propagated to widgets. The event loop follows a fixed dispatch order: registered key callbacks (`register_key`) fire first, then dialog input is processed, then clipboard actions, and finally spatial/focus-based dispatch to widgets.
