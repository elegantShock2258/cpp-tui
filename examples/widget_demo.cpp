...
inline void add_section(std::shared_ptr<Container> container,
...
inline std::shared_ptr<Button> make_theme_button(const std::string &label,
...
inline std::shared_ptr<Button> make_notif_button(
    const std::string &label, std::shared_ptr<Notification> notif,
    Notification::Type type, const std::string &message,
    Color bg_color = Color()) {
  auto btn = std::make_shared<Button>(
      label, [notif, message, type]() { notif->show(message, type, 3000); });
  btn->fixed_width = 10;
...
int main() {
  ...
  std::shared_ptr<Spinner> sp_dots_det, sp_line_det, sp_pie_det,
      sp_bounce_bar_det, sp_bounce_circle_det;
  ...
  // Header
  auto header =
      std::make_shared<Label>("cpp-tui Widget Showcase", Color{255, 200, 100});
  ...
  theme_layout->add(
      make_theme_button("Dark", []() { Theme::set_theme(Theme::Dark()); }));
  theme_layout->add(
      make_theme_button("Light", []() { Theme::set_theme(Theme::Light()); }));
  theme_layout->add(
      make_theme_button("Nord", []() { Theme::set_theme(Theme::Nord()); }));
  theme_layout->add(make_theme_button(
      "Tokyo", []() { Theme::set_theme(Theme::TokyoNight()); }));
  theme_layout->add(make_theme_button(
      "Solar", []() { Theme::set_theme(Theme::SolarizedLight()); }));
  ...
  {
    auto box =
        std::make_shared<Border>(BorderStyle::Rounded, Color{100, 180, 255});
    ...
    // --- Text Inputs ---
    sv->add(
        std::make_shared<Label>("[ Text Input ]", Theme::current().primary));
    ...
    // --- TextArea ---
    sv->add(std::make_shared<Label>("[ Multiple Line Text (TextArea) ]",
    ...
    ta->fixed_height = 6;
    ta->set_text(
        "This is a multi-line TextArea widget.\nIt supports scrolling, line "
        "numbers, and\nstandard text "
    ...
    // --- Toggle Switch ---
    sv->add(
        std::make_shared<Label>("[ Toggle Switch ]", Theme::current().primary));
    ...
    sv->add(std::make_shared<Label>("[ Checkboxes with Progress ]",
                                    Theme::current().primary));
    ...
    // --- Radio Buttons ---
    sv->add(
        std::make_shared<Label>("[ Radio Buttons ]", Theme::current().primary));
    ...
    drop->set_options({"Select Option...", "Apple", "Banana", "Cherry", "Date",
                       "Elderberry"});
    sv->add(drop);
    ...
    // --- Number Input ---
    sv->add(
        std::make_shared<Label>("[ Number Input ]", Theme::current().primary));
    ...
    // --- Checkbox List ---
    sv->add(
        std::make_shared<Label>("[ Checkbox List ]", Theme::current().primary));
    auto checklist = std::make_shared<CheckboxList>();
    checklist->set_options(
        {"Engine", "Transmission", "Wheels", "Nitro", "Spoiler"});
    sv->add(checklist);
    ...
    auto btn = std::make_shared<Button>(
        "Submit Form", [name_input, mail, result_label]() {
          result_label->set_text("Submitted: " + name_input->get_value() +
                                 " | " + mail->get_value());
        });
    ...
    // --- Styled Buttons ---
    auto section_label =
        std::make_shared<Label>("[ Styled Buttons ]", Theme::current().primary);
    sv->add(section_label);

    auto styled_btn_row = std::make_shared<Horizontal>();
    styled_btn_row->fixed_height = 1;

    auto btn_save = std::make_shared<Button>(
        StyledText().colored("S", Color::Cyan()).add(" Save"));
    btn_save->fixed_width = 12;
    styled_btn_row->add(btn_save);
    styled_btn_row->add(std::make_shared<HorizontalSpacer>(2));

    auto btn_delete = std::make_shared<Button>(
        StyledText().colored("X", Color::Red()).add(" Delete"));
    btn_delete->fixed_width = 14;
    styled_btn_row->add(btn_delete);
    styled_btn_row->add(std::make_shared<HorizontalSpacer>(2));

    auto btn_play = std::make_shared<Button>(
        StyledText().colored(">", Color::Green()).add(" Run"));
  ...
  {
    auto box =
        std::make_shared<Border>(BorderStyle::Rounded, Color{50, 200, 120});
    ...
    auto help = std::make_shared<Label>(
        "Controls: Arrows to nav, Space/Enter to toggle/submit",
        Color{150, 150, 150});
    help->fixed_height = 1;
  ...
  auto layouts_layout = std::make_shared<Vertical>();
  {
    // Alignments Section
    auto b_align =
        std::make_shared<Border>(BorderStyle::Single, Color{100, 200, 255});
    ...
    // Grid Section
    auto b_grid =
        std::make_shared<Border>(BorderStyle::Double, Color{255, 180, 100});
  ...
  auto scroll_layout = std::make_shared<Vertical>();
  {
    // Vertical Scroll
    auto v_box =
        std::make_shared<Border>(BorderStyle::Rounded, Color{0, 200, 200});
    ...
    // Horizontal Scroll
    auto h_box =
        std::make_shared<Border>(BorderStyle::Rounded, Color{255, 100, 100});
    ...
    // 2D Scroll
    auto box2d =
        std::make_shared<Border>(BorderStyle::Double, Color{150, 100, 255});
  ...
  {
    auto car_box =
        std::make_shared<Border>(BorderStyle::Double, Color{255, 180, 50});
    ...
    auto finish_btn = std::make_shared<Button>(
        StyledText().colored("✔", Color::Green()).add(" Finish"), []() {});
  ...
  {
    auto box =
        std::make_shared<Border>(BorderStyle::Double, Color{255, 100, 150});
    ...
    // Dialog demos
    v->add(
        std::make_shared<Label>("[ Dialog Demos ]", Theme::current().primary));
    ...
    auto ok_btn = std::make_shared<Button>("OK", [dlg1]() { dlg1->close(); });
    ...
    auto btn_open1 = std::make_shared<Button>("Open Info Dialog",
                                              [dlg1]() { dlg1->open(10, 5); });
    v->add(btn_open1);
    ...
    auto btn_submit =
        std::make_shared<Button>("Submit", [dlg2]() { dlg2->close(); });
    ...
    auto btn_cancel =
        std::make_shared<Button>("Cancel", [dlg2]() { dlg2->close(); });
    ...
    auto btn_open2 = std::make_shared<Button>("Open Modal Dialog",
                                              [dlg2]() { dlg2->open(15, 8); });
    v->add(btn_open2);
    ...
    auto btn_menu = std::make_shared<Button>("Open Context Menu",
                                             [menu]() { menu->open(40, 10); });
    v->add(btn_menu);

    v->add(std::make_shared<VerticalSpacer>(1));

    // Notifications Section
    v->add(std::make_shared<Label>("--- Notifications ---",
    ...
    h_notif->add(make_notif_button(
        "Success", notif, Notification::Type::Success,
        "Operation completed successfully!", Theme::current().success));
    h_notif->add(std::make_shared<HorizontalSpacer>(1));
    h_notif->add(make_notif_button(
        "Warning", notif, Notification::Type::Warning,
        "This is a warning notification", Theme::current().warning));
    h_notif->add(std::make_shared<HorizontalSpacer>(1));
    h_notif->add(make_notif_button("Error", notif, Notification::Type::Error,
                                   "An error has occurred!",
                                   Theme::current().error));

    v->add(h_notif);
    v->add(std::make_shared<Label>("Click buttons to show toast notifications",
    ...
    // Tooltip Section
    v->add(
        std::make_shared<Label>("--- Tooltip ---", Theme::current().secondary));
    ...
    v->add(tooltip_btn);
    v->add(std::make_shared<Label>("Hover over button to show tooltip",
                                   Theme::current().foreground));
  ...
  {
    auto box =
        std::make_shared<Border>(BorderStyle::Double, Color{255, 200, 50});
    ...
    // Custom Toggle
    sv->add(
        std::make_shared<Label>("[ Custom Toggle ]", Theme::current().primary));
    ...
    // Custom Radio
    sv->add(
        std::make_shared<Label>("[ Custom Radio ]", Theme::current().primary));
    auto radio = std::make_shared<RadioSet>();
    ...
    sv->add(std::make_shared<Label>("[ Custom Checkbox List ]",
                                    Theme::current().primary));
    ...
    sv->add(std::make_shared<Label>("[ Custom Progress Bars ]",
                                    Theme::current().primary));
    ...
    auto sp_circle =
        std::make_shared<Spinner>(&app, Spinner::StylePulseAscii());
    ...
    sp_bounce_bar_det =
        std::make_shared<Spinner>(&app, Spinner::StyleBarProgress());
    ...
    sv->add(std::make_shared<Label>("[ Responsive Visibility ]",
                                    Theme::current().primary));

    auto resp_large = std::make_shared<Label>(
        "Visible on LARGE WIDTH (>120 cols)", Color::Red());
    resp_large->set_responsive(false, false, true);
    sv->add(resp_large);

    auto resp_sm = std::make_shared<Label>("Visible on SMALL/MEDIUM WIDTH",
                                           Color::Green());
    ...
    auto resp_h = std::make_shared<Label>(
        "Visible on SMALL/MEDIUM HEIGHT (<50 lines)", Color::Red());
    resp_h->set_responsive_height(true, true, false);
    sv->add(resp_h);

    auto resp_h_lg = std::make_shared<Label>(
        "Visible on LARGE HEIGHT (>50 lines)", Color::Green());
  ...
  {
    auto box =
        std::make_shared<Border>(BorderStyle::Rounded, Color{100, 200, 100});
    box->set_title(" Text Styling & Paragraphs ", Alignment::Center);
    auto sv = std::make_shared<ScrollableVertical>();
    box->add(sv);

    // Section: Basic Text Styles
    sv->add(std::make_shared<Label>("[ Basic Text Styles ]",
                                    Theme::current().primary));
    sv->add(std::make_shared<VerticalSpacer>(1));

    // Bold paragraph
    auto p_bold =
        std::make_shared<Paragraph>("This text is bold. Bold text stands out.");
    p_bold->bold = true;
    p_bold->fixed_height = 1;
    sv->add(p_bold);

    // Italic paragraph
    auto p_italic = std::make_shared<Paragraph>(
        "This text is italic. Italics add emphasis.");
    p_italic->italic = true;
    p_italic->fixed_height = 1;
    sv->add(p_italic);

    // Underline paragraph (default: text-only)
    auto p_underline = std::make_shared<Paragraph>(
        "Underline: This only underlines the characters in the text.");
    ...
    sv->add(std::make_shared<Label>("[ Mixed Styled Text ]",
                                    Theme::current().primary));
    sv->add(std::make_shared<VerticalSpacer>(1));
    ...
    sv->add(std::make_shared<Label>("[ Paragraph Indentation ]",
                                    Theme::current().primary));
    sv->add(std::make_shared<VerticalSpacer>(1));

    // First-line indent (like a book paragraph)
    sv->add(std::make_shared<Label>("First-line indent (4 spaces):",
                                    Color{180, 180, 180}));
    ...
    sv->add(std::make_shared<Label>(
        "Hanging indent (4 spaces on subsequent lines):",
        Color{180, 180, 180}));
    ...
    sv->add(std::make_shared<Label>("[ Word Wrapping Demo ]",
                                    Theme::current().primary));
    sv->add(std::make_shared<VerticalSpacer>(1));

    auto p_wrap = std::make_shared<Paragraph>(
        "This paragraph demonstrates automatic word wrapping. As you resize "
        "the terminal, "
        "the text will reflow to fit the available width while respecting word "
        "boundaries. "
    ...
    // Section: Text Lists
    sv->add(
        std::make_shared<Label>("[ Text Lists ]", Theme::current().primary));
    sv->add(std::make_shared<VerticalSpacer>(1));

    // Bullet List
    sv->add(std::make_shared<Label>("Bullet List (default marker):",
                                    Color{180, 180, 180}));
    auto bullet_list = std::make_shared<TextList>();
    bullet_list->items = {ListItem{"First item in the list"},
    ...
    // Nested Bullet List
    sv->add(std::make_shared<Label>("Nested List (with levels):",
                                    Color{180, 180, 180}));
    auto nested_list = std::make_shared<TextList>();
    nested_list->items = {
        ListItem{"Fruits", 0},       ListItem{"Apples", 1},
        ListItem{"Granny Smith", 2}, ListItem{"Fuji", 2},
        ListItem{"Oranges", 1},      ListItem{"Vegetables", 0},
        ListItem{"Carrots", 1},      ListItem{"Broccoli", 1}};
    ...
    num_list->items = {ListItem{"Install dependencies"},
    ...
    sv->add(
        std::make_shared<Label>("Nested Numbered List:", Color{180, 180, 180}));
    auto nested_num = std::make_shared<TextList>();
    nested_num->items = {ListItem{"Getting Started", 0},
    ...
    sv->add(std::make_shared<Label>("List with Word Wrapping:",
                                    Color{180, 180, 180}));
    auto wrap_list = std::make_shared<TextList>();
    wrap_list->items = {ListItem{"This is a longer item that will wrap to the "
                                 "next line if the widget is narrow enough"},
                        ListItem{"Another item with extended text to "
  ...
  {
    // Title
    new_widgets_layout->add(std::make_shared<Label>(
        "=== Layout & Navigation Widgets ===", Theme::current().primary));
    new_widgets_layout->add(std::make_shared<VerticalSpacer>(1));

    // --- SearchInput Section ---
    new_widgets_layout->add(std::make_shared<Label>(
        "[ SearchInput - Type to filter ]", Theme::current().secondary));
    auto search = std::make_shared<SearchInput>(&app);
    search->placeholder = "Search fruits...";
    search->suggestions = {"Apple",      "Banana", "Cherry", "Date",
                           "Elderberry", "Fig",    "Grape",  "Honeydew",
                           "Kiwi",       "Lemon",  "Mango"};
    new_widgets_layout->add(search);

    auto search_result = std::make_shared<Label>(
        "Select a fruit and press Enter", Theme::current().border);
    ...
    // --- Breadcrumb Section ---
    new_widgets_layout->add(
        std::make_shared<Label>("[ Breadcrumb ]", Theme::current().secondary));
    auto bc = std::make_shared<Breadcrumb>();
    auto bc_result = std::make_shared<Label>("Click a breadcrumb to navigate",
    ...
    // --- ShortcutBar Section ---
    new_widgets_layout->add(
        std::make_shared<Label>("[ ShortcutBar ]", Theme::current().secondary));
    ...
    // --- StatusBar Section ---
    new_widgets_layout->add(
        std::make_shared<Label>("[ StatusBar ]", Theme::current().secondary));
    auto status = std::make_shared<StatusBar>();
    status->add_section(
        StyledText().colored_bold("●", Color::Green()).add(" ").bold("Ready"));
    ...
    status->add_section(StyledText()
                            .colored_bold("main", Color::Magenta())
                            .italic(" (branch)"));
  ...
  {
    accordion_layout->add(std::make_shared<Label>("=== Accordion Widget ===",
                                                  Theme::current().primary));
    accordion_layout->add(std::make_shared<VerticalSpacer>(1));

    accordion_layout->add(std::make_shared<Label>(
        "Use Up/Down arrows to navigate sections", Theme::current().secondary));
    accordion_layout->add(std::make_shared<Label>(
        "Press Enter or Space to expand/collapse", Theme::current().secondary));
    accordion_layout->add(std::make_shared<VerticalSpacer>(1));

    auto accordion = std::make_shared<Accordion>();
    accordion->add_section(
        "Section 1: Introduction",
        std::make_shared<Label>("  Welcome! This is the introduction section "
                                "with helpful content."));
    accordion->add_section(
        "Section 2: Features",
        std::make_shared<Label>("  Key features: Collapsible sections, "
                                "keyboard navigation, theming."));
    accordion->add_section(
        "Section 3: Usage",
        std::make_shared<Label>("  Use add_section() to add content. Set "
                                "allow_multiple for multi-expand."));
    accordion->add_section(
        "Section 4: API Reference",
        std::make_shared<Label>("  Properties: sections, selected_index, "
                                "header_bg, header_fg, icons."));
  ...
  auto selected_date_label = std::make_shared<Label>(
      "Selected Date: December 19, 2024", Theme::current().primary);
  {
    calendar_layout->add(std::make_shared<Label>("=== Calendar Widget ===",
                                                 Theme::current().primary));
    calendar_layout->add(std::make_shared<VerticalSpacer>(1));

    calendar_layout->add(std::make_shared<Label>(
        "Tab to < > buttons, or click them to change months",
        Theme::current().secondary));
    calendar_layout->add(std::make_shared<Label>(
        "Click on a day to select, then use arrows to navigate",
        Theme::current().secondary));
    calendar_layout->add(std::make_shared<Label>(
        "Press Enter to confirm selection", Theme::current().secondary));
    calendar_layout->add(std::make_shared<VerticalSpacer>(1));
    ...
    cal->on_select = [selected_date_label](int year, int month, int day) {
      ...
      selected_date_label->set_text(
          "Selected Date: " + std::string(months[month - 1]) + " " +
  ...
  {
    splitpane_layout->add(std::make_shared<Label>(
        "=== Resizable Split Pane ===", Theme::current().primary));
    splitpane_layout->add(std::make_shared<VerticalSpacer>(1));
    splitpane_layout->add(std::make_shared<Label>(
        "Drag the divider line to resize panes", Theme::current().secondary));
    splitpane_layout->add(std::make_shared<VerticalSpacer>(1));
    ...
    auto left_pane =
        std::make_shared<Border>(BorderStyle::Single, Theme::current().primary);
    ...
    auto right_pane = std::make_shared<Border>(BorderStyle::Single,
                                               Theme::current().secondary);
    ...
    splitpane_layout->add(std::make_shared<Label>(
        "Vertical Split (set vertical = true):", Theme::current().foreground));
    ...
    auto top_pane = std::make_shared<Border>(BorderStyle::Rounded,
                                             Theme::current().success);
    top_pane->set_title(" Top ", Alignment::Left);
    top_pane->add(
        std::make_shared<Label>("Top panel - drag the line to resize"));

    auto bottom_pane = std::make_shared<Border>(BorderStyle::Rounded,
  ...
  {
    color_layout->add(std::make_shared<Label>("=== Color Picker ===",
                                              Theme::current().primary));
    ...
    info_v->add(
        std::make_shared<Label>("Controls:", Theme::current().secondary));
    info_v->add(
        std::make_shared<Label>(" - Click on gradient to pick Hue/Sat"));
    ...
    picker->on_change = [selected_lbl](Color c) {
      ...
      selected_lbl->set_text("RGB: " + std::to_string(c.r) + "," +
  ...
  footer_bar->add_section(
      StyledText().colored_bold("[ / ]", Color::Cyan()).add(" Switch tabs"));
  footer_bar->add_section(
      StyledText().colored_bold("Arrows", Color::Yellow()).add(" Navigate"));
  footer_bar->add_section(
      StyledText().colored_bold("Enter/Space", Color::Green()).add(" Select"));
  footer_bar->add_section(
      StyledText().colored_bold("q", Color::Red()).add(" Quit"));
  ...
  app.add_timer(80, [&]() {
    ...
    auto update = [&](std::shared_ptr<Spinner> s, std::shared_ptr<Label> l,
                      std::string prefix) {
