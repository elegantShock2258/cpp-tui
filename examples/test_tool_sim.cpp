...
enum class TestStatus { Pending, Running, Passed, Failed, Skipped };
 ...
 public:
      ...
      if (t.status == TestStatus::Pending) {
        ...
        if (rand() % 10 > 2) {              // 70% chance to finish this tick
          ...
          if (pass) {
            ...
            t.output_log += "[FAIL] Assertion error at line " +
  ...
  void refresh_table() {
    ...
    for (const auto &t : tests) {
      ...
      test_table->add_row({std::to_string(t.id), t.suite, t.name, s_str,
                           std::to_string((int)t.duration_ms) + "ms"});
    }
  }
  ...
  void update_stats() {
    if (status_label) {
      std::stringstream ss;
      ss << "Total: " << tests.size() << " | Passed: " << count_pass
         << " | Failed: " << count_fail << " | Pending: "
         << (tests.size() - count_pass - count_fail - count_skip);
      status_label->set_text(ss.str());
    }
    if (total_progress) {
      float progress =
          (float)(count_pass + count_fail + count_skip) / (float)tests.size();
  ...
  void show_output_for(int row_index) {
    ...
    // Map row index to test (assuming simplistic mapping where row index
    // matches test index if no filter) For accurate mapping with filters, we'd
    // need more logic. Assuming ALL filter for Demo simplicity.
    if (row_index >= 0 && row_index < tests.size()) {
      const auto &t = tests[row_index];
      output_view->add(std::make_shared<Label>("Output for: " + t.name,
...
int main() {
  ...
  // Header
  auto header = std::make_shared<Label>(" TEST RUNNER SIMULATION ",
                                        Theme::current().primary);
  ...
  auto btn_run = std::make_shared<Button>("Run All Tests",
                                          [runner]() { runner->start_all(); });
  ...
  auto btn_reset =
      std::make_shared<Button>("Reset", [runner]() { runner->reset_all(); });
  ...
  app.add_timer(100, [runner]() {
    ...
    // Better: Only refresh output if the running test is the one currently
    // being viewed.
    if (runner->is_running && runner->current_running_index >= 0) {
      if (runner->test_table &&
          runner->test_table->selected_index == runner->current_running_index) {
