...
// =============================================================================
static std::shared_ptr<Dialog> make_dialog(
    App *app, const std::string &title, const std::string &message) {
  ...
  auto msg_row = std::make_shared<Align>(Align::H::Center, Align::V::Center);
  msg_row->add(std::make_shared<Label>(message));
  content->add(msg_row);
...
int main() {
  ...
  // Title (centred)
  {
    ...
    auto title_lbl = std::make_shared<Label>(title_text, Theme::current().primary);
  ...
  // Footer (centred)
  {
    ...
    auto footer_lbl = std::make_shared<Label>(footer_text, Theme::current().border);
  ...
  // Ctrl+Shift+H
  app.register_key('h', [dlg_palette] { dlg_palette->open(); },
                   /*ctrl=*/true, /*alt=*/false, /*shift=*/true);
  app.register_key('H', [dlg_palette] { dlg_palette->open(); },
                   /*ctrl=*/true, /*alt=*/false, /*shift=*/true);

  // Ctrl+O       (ctrl only, no shift)
  app.register_key('o', [dlg_open] { dlg_open->open(); },
                   /*ctrl=*/true, /*alt=*/false, /*shift=*/false);
  app.register_key('O', [dlg_open] { dlg_open->open(); },
                   /*ctrl=*/true, /*alt=*/false, /*shift=*/false);

  // Ctrl+S       (ctrl only, no shift)
  app.register_key('s', [dlg_save] { dlg_save->open(); },
                   /*ctrl=*/true, /*alt=*/false, /*shift=*/false);
  app.register_key('S', [dlg_save] { dlg_save->open(); },
                   /*ctrl=*/true, /*alt=*/false, /*shift=*/false);

  // Ctrl+F       (ctrl only, no shift)
  app.register_key('f', [dlg_find] { dlg_find->open(); },
                   /*ctrl=*/true, /*alt=*/false, /*shift=*/false);
  app.register_key('F', [dlg_find] { dlg_find->open(); },
                   /*ctrl=*/true, /*alt=*/false, /*shift=*/false);

  // Ctrl+G       (ctrl only, no shift)
  app.register_key('g', [dlg_goto] { dlg_goto->open(); },
                   /*ctrl=*/true, /*alt=*/false, /*shift=*/false);
  app.register_key('G', [dlg_goto] { dlg_goto->open(); },
                   /*ctrl=*/true, /*alt=*/false, /*shift=*/false);

  // Alt+X        (alt only, no ctrl/shift)
  app.register_key('x', [dlg_run] { dlg_run->open(); },
                   /*ctrl=*/false, /*alt=*/true, /*shift=*/false);
  app.register_key('X', [dlg_run] { dlg_run->open(); },
                   /*ctrl=*/false, /*alt=*/true, /*shift=*/false);

  // Ctrl+Alt+D   (ctrl + alt, no shift)
  app.register_key('d', [dlg_debug] { dlg_debug->open(); },
