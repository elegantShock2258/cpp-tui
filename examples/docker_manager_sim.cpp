...
enum class ContainerState { Running, Stopped, Paused, Restarting };
...
class DockerManager : public Border {
 public:
  ...
  DockerManager() : Border(BorderStyle::Double) {
    ...
    // Init Dummy Data
    containers = {
        {"1a2b3c", "web-frontend", "nginx:latest", ContainerState::Running,
         0.5f, 2.5f, "0.0.0.0:80->80/tcp", "Up 2 hours"},
        {"4d5e6f", "api-gateway", "traefik:v2", ContainerState::Running, 1.2f,
         4.0f, "0.0.0.0:443->443/tcp", "Up 2 hours"},
        {"7g8h9i", "db-primary", "postgres:14", ContainerState::Running, 4.5f,
         15.2f, "5432/tcp", "Up 5 days"},
        {"0j1k2l", "cache-redis", "redis:alpine", ContainerState::Running, 0.1f,
         0.8f, "6379/tcp", "Up 5 days"},
        {"3m4n5o", "worker-job", "python:3.9-slim", ContainerState::Stopped,
         0.0f, 0.0f, "", "Exited (0) 10 min ago"},
        {"6p7q8r", "dev-sandbox", "ubuntu:22.04", ContainerState::Paused, 0.0f,
         4.2f, "", "Paused"},
        {"9s0t1u", "metrics-prom", "prom/prometheus", ContainerState::Running,
         2.1f, 6.5f, "9090/tcp", "Up 1 day"},
        {"2v3w4x", "grafana-ui", "grafana/grafana", ContainerState::Running,
         1.5f, 5.8f, "3000/tcp", "Up 1 day"}};
  }
  ...
  void tick() {
    ...
    if (tick_count % 10 == 0) {
      ...
      for (auto &c : containers)
        if (c.state == ContainerState::Running) running.push_back(&c);

      if (!running.empty()) {
        auto *lucky = running[rand() % running.size()];
        std::stringstream ss;
        ss << "[" << lucky->name << "] "
           << "GET /api/v1/resource " << (rand() % 400 + 200) << " "
           << (rand() % 100) << "ms";
  ...
  void action_start() {
    ...
    if (notification)
      notification->show("Container " + target_id + " started",
                         Notification::Type::Success);
  }
  void action_stop() {
    update_selected_state(ContainerState::Stopped);
    if (notification)
      notification->show("Container " + target_id + " stopped",
  ...
  void action_restart() {
    ...
    update_selected_state(ContainerState::Running);
    if (notification)
      notification->show("Container " + target_id + " restarted",
  ...
  void refresh_table() {
    ...
    for (const auto &c : containers) {
      ...
      container_table->add_row({c.id, c.name, c.image, state_to_str(c.state),
                                cpu.str(), mem.str(), c.ports, c.status_msg});
    }

    if (stats_label) {
      stats_label->set_text("Running: " + std::to_string(running_cnt) + " / " +
                            std::to_string(containers.size()));
    }
  }
};
...
int main() {
  ...
  // Also allow on_change for immediate feedback if desired, but search usually
  // implies selection
  ...
  auto btn_start = std::make_shared<Button>(
      " Start ", [manager]() { manager->action_start(); });
  btn_start->bg_color = Theme::current().success;
  tb_in->add(btn_start);

  auto btn_stop = std::make_shared<Button>(
      " Stop ", [manager]() { manager->action_stop(); });
  btn_stop->bg_color = Theme::current().error;
  tb_in->add(btn_stop);

  auto btn_restart = std::make_shared<Button>(
      " Restart ", [manager]() { manager->action_restart(); });
  ...
  table->columns = {"ID",  "Name", "Image", "State",
                    "CPU", "MEM",  "Ports", "Status"};
  ...
  root->add(std::make_shared<Label>(
      "Press 'q' to quit. Enter Target ID to control containers.",
      Color{100, 100, 100}));

  // Add Notification widget to tree so it gets render calls
  root->add(notif);

  app.add_timer(500, [manager]() { manager->tick(); });
