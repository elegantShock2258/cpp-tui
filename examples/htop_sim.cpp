 ...
 public:
  ...
  void tick() {
    ...
    for (size_t i = 0; i < cpus.size(); ++i) {
      // Skewed random usage
      float val = 0.3f + 0.3f * std::sin(time + i * 1.5f) +
                  0.2f * ((std::rand() % 100) / 100.0f);
    ...
    if (top_status) {
      ...
      top_status->sections[0].styled_content =
          StyledText().colored("⏲", Color::Cyan()).add(ss_up.str());

      float l1 = 0.5f + 0.2f * std::sin(time * 0.1f) +
                 0.1f * ((std::rand() % 10) / 10.0f);
      float l5 = l1 * 0.9f + 0.05f * std::cos(time * 0.05f);
      float l15 = l5 * 0.8f + 0.1f;

      std::stringstream ss_load;
      ss_load << " " << std::fixed << std::setprecision(2) << l1 << " " << l5
              << " " << l15;
      top_status->sections[1].styled_content =
          StyledText()
  ...
  bool on_event(const Event &e) override { return Border::on_event(e); }
};
...
int main() {
  ...
  theme_layout->add(std::make_shared<Label>(
      ""));  // Spacer to push button to left if desired, or just let it be
  ...
  list_body->columns = {"PID",  "USER", "PRI",  "NI",
                        "VIRT", "CPU%", "MEM%", "Command"};
  ...
  for (const auto &p : processes) {
    ...
    list_body->add_row({std::to_string(p.pid), p.user, "20", "0", "106M",
                        cpu_ss.str(), mem_ss.str(), p.cmd});
  }
  ...
  // --- Timer ---
  app.add_timer(200, [&]() { sim_root->tick(); });
