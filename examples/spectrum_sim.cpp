...
int main() {
  ...
  auto header = std::make_shared<Label>(
      "Real-time Spectrum Analyzer Simulation", Color{0, 255, 255});

  header->fixed_height = 1;
  root->add(header);

  auto info_box =
      std::make_shared<Border>(BorderStyle::Single, Color{100, 100, 100});
  auto info_txt = std::make_shared<Label>(
      "Simulating RF Spectrum: 2.4GHz Band | Noise Floor: -90dBm | Peaks: "
      "Bluetooth/WiFi");
  ...
  // X-axis: Frequency in MHz (2.4GHz band: 2400-2500 MHz mapped across 200
  // bins)
  ...
  chart->tooltip_formatter = [&](const LineChart::Series &s, int index,
    ...
    return s.label + ": " + std::to_string(mhz) + "MHz " + std::to_string(dbm) +
           "dBm";
  };
  ...
  app.add_timer(30, [&]() {  // Faster refresh 30ms ~30fps
    ...
    for (int i = 0; i < bins; ++i) {
      ...
      if (std::fmod(time_t, 3.0) > 2.2) {
        ...
        // Wideband noise
        p += gaussian((double)i, wifi_center + 10.0 * std::sin(time_t * 50),
                      10.0, 0.3 * (rand() % 100 / 100.0));
      }
    ...
    chart->add_series(max_hold, "Max Hold", {255, 100, 100},
                      LineChart::LineStyle::Braille);

    // Average (Yellow, Braille)
    chart->add_series(avg_line, "Avg", {255, 255, 0},
                      LineChart::LineStyle::Braille);

    // Current (Green, Braille, Filled)
    chart->add_series(power, "Live", {0, 255, 0},
                      LineChart::LineStyle::Braille);
