...
/// @brief Demonstrates thread-safe UI updates from background threads using
/// App::post()
///
  ...
  auto start_task = [&]() {
    ...
    std::thread worker([&]() {
      ...
      std::vector<std::string> files = {
          "package-v2.3.1.tar.gz", "runtime-libs-x64.deb", "assets-hd.zip",
          "documentation.pdf", "config-templates.tar"};
      ...
      for (size_t i = 0; i < files.size(); ++i) {
        ...
        for (int step = 0; step < steps; ++step) {
          ...
          app.post([&, overall]() { progress->value = overall; });
        }

        app.post([&, file]() {
          log_area->set_text(log_area->get_text() + "  ✓ " + file +
                             " complete\n");
        });
      }
