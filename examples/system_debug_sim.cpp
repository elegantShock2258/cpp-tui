...
class SystemSimulator : public Border {
 public:
  ...
  void tick() {
    ...
    if (current_cpu > 1) current_cpu = 1;

    current_mem += (eff_mem_target - current_mem) * 0.1f + noise * 0.5f;
    if (current_mem < 0) current_mem = 0;
    if (current_mem > 1) current_mem = 1;
...
int main() {
  ...
  // --- Top: Header ---
  auto header = std::make_shared<Label>(" SYSTEM DEBUG TOOL v1.0 ",
  ...
  auto footer = std::make_shared<Label>(
      " [q] Quit  |  [Tab] Next Control  |  [Space] Activate ",
      Color{100, 100, 100});
  ...
  // Timer
  app.add_timer(500, [system_sim]() { system_sim->tick(); });
