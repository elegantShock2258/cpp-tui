...
int main() {
  ...
  file_menu->items.push_back({"New", [&text_area, notif]() {
                                text_area->set_text("");
                                notif->show("New file created.",
                                            Notification::Type::Info);
                              }});

  file_menu->items.push_back({"Save", [&text_area, notif]() {
                                notif->show("File Saved (Simulated).",
                                            Notification::Type::Success);
                              }});

  file_menu->items.push_back({"Exit", []() { App::quit(); }});

  menu_bar->items.push_back(
      {"File", [file_menu, &app]() { file_menu->open(0, 1); }});
  ...
  std::string wrap_label =
      text_area->word_wrap ? "Turn Word Wrap OFF" : "Turn Word Wrap ON";

  view_menu->items.push_back({wrap_label, [text_area, view_menu]() {
                                text_area->word_wrap = !text_area->word_wrap;
                                // Update the menu item label for next time it's
                                // opened
                                view_menu->items[0].label =
                                    text_area->word_wrap ? "Turn Word Wrap OFF"
                                                         : "Turn Word Wrap ON";
  ...
  std::string ln_label =
      text_area->show_line_numbers ? "Hide Line Numbers" : "Show Line Numbers";

  view_menu->items.push_back({ln_label, [text_area, view_menu]() {
                                text_area->show_line_numbers =
                                    !text_area->show_line_numbers;
                                // Update the menu item label (index 1)
                                view_menu->items[1].label =
                                    text_area->show_line_numbers
                                        ? "Hide Line Numbers"
                                        : "Show Line Numbers";
                                // Force cursor visibility check as dimensions
                                // change
                                text_area->ensure_cursor_visible();
                              }});
  ...
  std::string tab_label =
      text_area->accepts_tab ? "Disable Tab Indent" : "Enable Tab Indent";
  view_menu->items.push_back({tab_label, [text_area, view_menu]() {
                                text_area->accepts_tab =
                                    !text_area->accepts_tab;
                                view_menu->items[2].label =
                                    text_area->accepts_tab
                                        ? "Disable Tab Indent"
                                        : "Enable Tab Indent";
                              }});
  ...
  status_bar->add_section(
      StyledText().colored_bold("●", Color::Green()).add(" Ready"), 15);
  ...
  text_area->on_cursor_move = [status_bar](int x, int y) {
    status_bar->sections[2].styled_content =
        "Ln " + std::to_string(y + 1) + ", Col " + std::to_string(x + 1);
  };
  ...
  explorer->on_file_selected = [&](std::string path) {
    ...
    if (f) {
      ...
      status_bar->sections[0].styled_content =
          StyledText()
              .colored_bold("✏", Color::Yellow())
              .add(" ")
              .add(std::filesystem::path(path).filename().string());
    }
  };
